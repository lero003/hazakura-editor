...
| **Basic packaged smoke passed / extended partial** | **Reference Compare packaged proof (T-3)** | Signed build 85 metadata/signature/helpers verified; user-reported basic packaged testing found no issue; latest source-built sandbox preview passed app/helper entitlement and deep-signature checks on 2026-07-12. | Keep picker interaction, recovery, long-reference boundary, and a11y matrix as packaged/manual follow-up evidence. |
