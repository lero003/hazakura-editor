...
| **Developer packaged smoke passed / source close-replace wiring + image/PDF a11y contract + PDF/image proof partial** | **Reference Compare packaged proof (T-3)** | Signed build 85 metadata/signature/helpers verified; user-reported basic packaged testing found no issue; latest source-built sandbox preview passed app/helper entitlement and deep-signature checks on 2026-07-12. A separate-ID Developer bundle opened a disposable workspace and preserved a Markdown/text reference across Preview → Reference; on 2026-07-13 it also opened `hazakura-valid-text.pdf` and workspace `reference-image.png` as read-only right-side content without changing the center buffer. On 2026-07-13 it additionally closed/reopened the image, replaced it with a nine-page PDF, moved to page 2, exercised fit-page plus 150% controls, and narrowed the Reference column to 25% while the center marker stayed intact. `AppWorkspace` now pins that close/replace actions do not call the editor change path; `ReferenceTextPane` and `ReferencePdfPane` keep direct read-only/accessibility contract tests. | Keep signed TestFlight interaction, source-deletion, long-reference copy/breadth, and the remaining a11y matrix as packaged/manual follow-up evidence. |
