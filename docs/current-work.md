...
2. **2.9-03（ローカル実装済み）:** System操作/可用性検査をstream/非streamで共通化。
   string wireを維持し、context/利用不可/言語/取消/timeout/混雑の内部分類と3言語の案内を追加。
   関連128件・型検査・Vite、fixture/live universal helperとprobe、6要求のstream/非stream互換確認が成功。
