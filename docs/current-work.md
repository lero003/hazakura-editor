...
| **Developer packaged smoke passed / PDF proof extended partial** | **Reference Compare packaged proof (T-3)** | Signed build 85 metadata/signature/helpers verified; user-reported basic packaged testing found no issue; latest source-built sandbox preview passed app/helper entitlement and deep-signature checks on 2026-07-12. A separate-ID Developer bundle opened a disposable workspace through the native picker and preserved a Markdown/text reference across Preview → Reference; on 2026-07-13 it also opened `hazakura-valid-text.pdf` as a read-only right-side page image without changing the center buffer. | Keep signed TestFlight interaction, image/reference breadth, long-reference copy, and a11y matrix as packaged/manual follow-up evidence. |
