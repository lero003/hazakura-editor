...
| # | やること | 種別 |
| --- | --- | --- |
| 0 | **画面20の明暗を決める**（モックの明色へ寄せるか、現行の薄暮を維持するか） | **オーナー判断** |
| 1 | 江戸彼岸オーバーレイの z をモーダルより下へ（1行級の不具合） | 実装 |
| 2 | スライスD: 14の2版カード・24の倍率/fit | 実装 |
| 3 | スライスE: 05・11・12・13 の専用面 | 実装（量大） |
| 4 | スライスC: 06・07・21・22 の見た目・文言整合 | 実装 |
