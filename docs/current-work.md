...
| **Developer packaged dialog smoke / partial** | **Export/theme polish (P2)** | EPUB/PDF dialogs now summarize unsaved-source inclusion, image-warning policy, and the next Save-dialog destination step; PDF final success preserves image-warning count. Spellcheck already lives in Preferences and reduced-motion source guards are present. | Keep actual EPUB/PDF output inspection, signed TestFlight breadth, and measured shader/theme work as follow-up. |
