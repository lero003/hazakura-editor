...
Status: Operational
Scope: v3.2依存更新キュー、次期メモ・縦書き方針、v3.1以前の記録
Authority: High
Last reviewed: 2026-09-26

## 現行キュー — v3.2.0依存関係更新と品質強化（2026-09-26）

v3.1はApple審査を通過して公開済み（オーナー報告）。下のv3.1節は当時の
source・候補・未完了ゲートの履歴であり、現在の公開状態を示すものではない。
次版はnpm/Rustの依存関係更新を主題とする。通常の互換更新とメジャー更新を
個別に検証し、最終的には同一候補のmacOS実機受け入れで判断する。
具体的な更新対象・例外・受け入れ項目は
[v3.2依存受け入れドラフト](releases/3.2.0-dependency-acceptance-draft.md)。
CodeMirror長文日本語/L Mode、HTTPS画像、ウィンドウ表示、Diff→Apply→Undoを
優先する。署名済みTestFlight/Apple審査/公開はsourceテストと分けて記録する。
2026-09-25の候補はnpm/Rust全テスト、監査、両配布プレビューとad-hoc sandboxの
静的smokeを通過。隔離Developerアプリの長文日本語・L Mode・保存・Reader実操作は
一部通過し、長文EPUB/PDF出力とHTTPS画像の表示・EPUB同梱も確認。
Reader復帰の初回位置だけ追加確認中。結果と残ゲートは上記ドラフトに記録。

2026-09-26の品質強化では、保存・外部変更監視・開き直し・バックアップ一覧の
遅延応答による状態上書きを再現して修正。AppShell、EditorPane、PDF/HTML/EPUBの
責務を分離し、Assist停止を待たないメニュー終了と、停止待ち中の編集を見落とす終了も修正した。
...
終了時のAssist停止と未保存確認を上記マトリクスに沿って受け入れる。

## v3.2後の方向 — メモと縦書き（2026-09-26、方針採用）

オーナー決定で、見直しメモ・再開メモ・編集ルールメモと縦書きをロードマップへ採用した。
現行v3.2の受け入れ後、メモ系は単一原稿の見直しメモを起点に、再開メモ、
自由記述の編集ルールひな形へ進める。表記の決め事もメモとして書き足す形にする。
縦書きは単一原稿のReaderから既存EPUBの縦書き出力へ段階化し、LLM拡張を前提にしない。

次の設計では、メモの保存・原稿との紐付け・移動・削除・位置を失ったときの扱いを固定する。
AIによるひな形・メモ内容の候補作成と、メモを使った本文推敲は別スライス。
モデル品質の再評価後、対象を明示して候補を確認・採用する流れへ接続する。
機能実装・版割り・出荷日は未確定。採用方針は[roadmap](roadmap.md)、
具体例と受け入れ条件は[計画詳細](post-v3.1-writing-completion-draft.md)を参照する。
...
ファイルの検証へ入る時点で`Verifying`を画面に通知し、不定進捗と編集継続の案内を表示する。
途中通知と最終確定の両方で操作世代を照合し、古い取得の結果で現在の状態を上書きしない。
検証中には「ダウンロードのキャンセル」を出さない。ハッシュ計算を止めて途中通知と古い操作の
抑止を確認するRust回帰テストを追加した。

「削除して再取得」の確認文は対象が選択中かで分け、選択中ならApple Intelligenceへ選択が
切り替わり、再取得後に利用者が「使う」で選び直すことを明示する。取得済みモデルの操作は
「更新を確認して取得」とし、新版があればダウンロードする説明を添えた。Local Assistの
モデル一覧では選択できない理由と取得進捗を短く表示し、読み上げ名にも含める。モデルの
自動選択・自動適用は加えていない。「短い例文で試す」の文書非使用経路も維持する。

frontend 2719件・scripts 31件、Rust 480件pass / 3件ignored、App Store surface 134件、
Rust fmt、ローカル`npm run build`のApp Storeプレビューが通過。実Background Assetsの
SHA-256所要時間、署名済みTestFlightの表示、VoiceOverでの読み上げは未確認。
**build 154の署名済みpkgは今回の仕上げを含まない。** 次の署名済み候補で12Bの取得→検証→
選択、再取得、更新、Local Assist一覧の読み上げまで受け入れる。
...
無効にする条件ではない。既存のnative経路は選択されたCore AIモデルを別probeで読み込む。
今回、アシスト設定からモデル管理へ進める導線、Systemモデルが無効な時のモデル別案内、
選択モデルのID・状態・pack version変更時の再probeを追加した。未選択モデルのDL進捗は
再probeしない。ダウンロードだけでモデル選択やLocal Assistの有効化は行わず、ユーザーが
Local Assistを選び再起動し、モデル管理で利用可能なCore AIモデルを選ぶ。macOS 27以降の
Apple Silicon Macで選択モデルのprobeが成功した場合だけ生成操作を使えるようにする。
Systemモデルに戻してApple IntelligenceがOFFなら`disabled`に戻る。

frontend 2716件・scripts 31件、型検査、Vite build、App Store surface 134件、
ローカル`npm run build`を通した。
Apple Intelligenceを実際にOFFにした署名済みTestFlightでの
12B取得・選択・通常/streaming生成・System復帰は未実施。**build 154のpkgはこの追補を含まない。**
