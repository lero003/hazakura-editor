...
| P1 done | Preview copy and companion UI polish | User-facing Local Assist copy uses `Preview` / `プレビュー` instead of `Alpha`, `Experimental`, or `実験的`; availability copy now gives light-user guidance for macOS 26+, M1+ Mac, Apple Intelligence enablement, and supported language / region. The detached companion now opens as a compact tool window, keeps the request form vertically tight, and uses a short scrollable in-session progress log instead of a tall empty panel. |
