...
Status: Operational
Scope: focus ringとガター・領域境界の調整、設定外枠と画面16/05/23/24、native受入
Authority: High
...
1. ~~「実装途中感」の3点~~ / ~~focus ringと薄め~~ / ~~ガター境界~~ / ~~chromeと紙面の境目の線~~ **完了**（上記）。
2. ~~設定の外枠寸法（1100×752・レール200px）~~ **完了**（上記）。Help導線は未決（パレットを任意カテゴリで開くAPIが無い）。
3. 画面23（狭い窓）：1024でプレビューは自動で隠れるがサイドバーは280pxで開いたまま。一時折畳みと利用者設定の分離が要る。
4. 画面16（文字と表示）：4サイズ設定はある。「本文書体」「行間」の独立設定は未追加（モックも別タスクと記載）。
5. 画面24（画像）：`ImagePreviewPane` 既存。参考寸法（上部59px・情報列54px）との差は未計測。
