...
| Done / v1.5 | L Mode 切替時の EditorView 再マウント（CodeMirror tile tree 破損回避） | **事象**: 行数が多く 1 画面に収まらない `.md` で L Mode ↔ edit を行き来すると、特定行より下が表示されなくなる。繰り返すと webview 全体がベタ塗りになりシステムボタン（閉じる / Cmd+Q）も効かなくなるハングに至る。別ファイルを開いて戻ると復活する。**原因**: `@codemirror/view` 6.43.3 / 6.43.4 で入った content DOM update / tile tree（行仮想化・描画範囲計算の中核）修正が、L Mode の重い block/replace decoration を Compartment reconfigure で差し替える経路と組み合わさって tile tree を破損する。`requestMeasure()` や CSS の `EditorView.theme` 移管では回復しない（tile tree 破損は再測定では治らない）。別 worktree で `@codemirror/view` を 6.43.2 に戻したら症状が消えたことで確定。**対策**: `EditorPane.tsx` の `shouldRemount` に `mountedLModeEnabledRef.current !== lModeEnabled` を追加し、L Mode ON/OFF のたびに EditorView を作り直して tile tree を新規構築する。カーソル位置とスクロール比は直前の View から吸い上げて復元（タブ切替時は引き続き `editorViewState` prop を優先）。**代償**: L Mode 切替で Undo 履歴がリセットされる（EditorView 作り直しのため）。ユーザー合意済み。依存は最新（6.43.5 / state 6.7.0）を維持。将来 CodeMirror を更新する際、この制約（L Mode 切替で再マウントが必要）を残すべきか見直すこと。 |
...
| Observation | v1.2 Image / Review Edge Cases | Take one case at a time: consecutive e-book images, L Mode first-line/nested-path images, PDF page breaks through a large image, explicit Diff `採用` / `破棄`, or unexpected `.bak` files in Trash. Do not bundle these into a polish slice. The explicit Diff `採用` / `破棄` stale-detection case is implemented: buffer changes after a change-review snapshot are detected and surfaced as a stale warning (`src/features/diff/changeReviewStale.ts`), with locale copy in `src/lib/locale/review.ts` and focused tests. The remaining image / `.bak` cases stay observation-only until a concrete reproduced gap appears. |
...
Book Scope / Book Workspace belongs after that footing is proven. Its
design direction, OKF as a structural foundation, the open UI questions,
and UI candidates that are not yet decided live in
`docs/superpowers/specs/2026-07-02-v2-book-scope-design.md`; treat that
spec as the single source of truth for v2 Book Scope design.
...
| `useAppShellController` orchestration | Keep intact for v1 unless one named app-shell flow breaks. Prefer focused leaf extraction or regression tests over splitting the top-level hook for size alone. Now also an observability candidate (see below): extracting transaction / render-measurement seams is allowed when it lets a focused test or error trace cover a previously hard-to-isolate flow. | A concrete New/Open/Save/Recovery/reader-mode transition bug needs isolation, or a reproduced issue cannot be localized because the relevant state is buried in the orchestrator. |
| Preview / e-book render pipeline (performance) | Allow a structural performance review as a v1.x slice. Target redundant `renderMarkdown` passes, unnecessary DOM mutations, and rAF / measurement churn on long manuscripts. Keep the single source-preserving parse -> sanitize -> DOM pipeline; do not add Preview DOM editing, virtualization that changes reading flow, or a second render path. | A long-manuscript fixture shows measurable re-parse / DOM churn, or Preview / e-book scroll / re-render stutter is reproduced on a real document. |
| Observability / diagnosability seams | Allow refactor-for-debuggability across `useAppShellController`, Local Assist transaction state, and the render / measurement path. Expose named seams (extracted helpers, status / error trace points, focused regression tests) so a reproduced issue localizes faster. No public-behavior change, no Markdown source change, no App Store lane change, and no new telemetry / network reporting. | A reproduced issue is hard to localize because state is concentrated without named seams, or a golden-path flow is too brittle to cover without restructuring. |
| Hazakura Local Assist companion | Observation only. Split request/progress/review state only if built-app smoke exposes responsiveness, availability, or transaction-boundary instability. The helper lifecycle teardown slice (Active UX Queue) is separate and targets deterministic shutdown on window / tab close. | Local Assist availability, generation, Diff / Discard, or unsaved-transaction behavior regresses. |
