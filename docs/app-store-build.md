...
- Latest local App Store package candidate metadata (version, build
  ...
  longer carry per-build values; consult `latest.json` instead. The
  prior `0.34.0` build `46` candidate is superseded for PDF output
  because TestFlight still showed macOS' print-unsupported alert.
