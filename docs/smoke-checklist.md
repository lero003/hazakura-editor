...
3. For a manuscript in a child folder with `../assets/cover.png`, open the project parent as the workspace and confirm the image appears in Preview, whole-book Reader, exported HTML, and PDF. Keep Preview open for at least 10 seconds, then close/reopen the pane and confirm the visible image neither flashes away nor returns to a blank reserved frame. Move e-book to the actual image page and wait another 10 seconds. Reopen only the child folder and confirm all four surfaces show a blocked note with **outside-workspace** reason and parent-folder next action (Theme G M0/M1). Also confirm a drag-dropped `assets/...` image loads and a missing workspace image shows a **load-failed** note.
...
18. Open Settings, use the on-device model entry, and confirm the dialog switches to the model page without closing (the header selector must also list it). **Keyboard: focus must land on the new page heading before the next Tab, and switching from the header selector must keep focus on the selector.** Confirm the system menu also opens the page directly (app menu on macOS next to Preferences…, File menu elsewhere) and that Kana shows its own label. The page shows the model list with status, size, and asset version, the storage explanation, and the generation-settings panel; before any Core AI run that panel shows its empty state. Run one Core AI generation, reopen the page, and confirm the requested and effective sampling, the output cap, the last token counts, and the model that ran match the harness report for the same build. A panel that still shows an older model's values means the record is not being refreshed.
