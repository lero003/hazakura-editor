...
Latest v0.10.0 release-candidate smoke focus: confirm `Cmd+Shift+L` L Mode entry/exit, Markdown source preservation, long-document scrolling, keyboard line focus, inactive Markdown marker suppression, active/hovered marker reveal, reference-style link marker hiding, code-block readability, theme-aware floating status text, workspace `+` create actions, folder context-menu create actions, inline Rename, Move to Trash confirmation, Agent provider availability states, and Safe Editor boundary absence of Git UI, arbitrary command/path fields, or auto-apply behavior. Workspace-internal drag/drop Move remains user-reported unreliable and should be treated as experimental until built-app smoke proves it dependable.
...
## v0.5 Image Asset / Export / Workbench Smoke

Planned for 2026-05-31. Run these on the built DMG or built `.app` bundle.
...
Latest Agent Workbench OpenCode right-pane resize note: 2026-05-30 user smoke reported OpenCode TUI rendering broke after resizing the right pane. Automated gates passed after keeping xterm as a live stream when bounded output history skips chunks and after notifying the provider process with `SIGWINCH` when the backend PTY size changes. Real OpenCode resize still needs trusted-workspace manual smoke.
...
Latest Review Desk visible shell checks: 2026-06-01 automated gates (`npm run typecheck`, `npm run build:vite`, `git diff --check`) passed after adding the visible Review Desk shell, the `Cmd+Shift+R` toggle, and the `useReviewDeskState` / `useReviewDeskController` skeleton. Built-app smoke confirmed Review Desk opens, closes, and returns to the existing workspace surface.

Latest Review Desk manual candidate MVP checks: 2026-06-01 automated gates (`npm run typecheck`, `npm run build:vite`, `git diff --check`, `cargo test --manifest-path src-tauri/Cargo.toml`, `npm run build`) passed after adding the manual candidate paste area, the `kind: "candidate"` `CompareCase` branch, the `runCandidateCompare` hook, the localized candidate copy, and the new `.review-surface-candidate-*` CSS rules. Built-app smoke confirmed manual candidate paste, Compare diff preview, Clear, and Close.
...
Latest Review Desk candidate edit safety checks: 2026-06-01 code inspection found that editing the manual candidate textarea after a successful Compare left the old preview and Apply candidate action available. The input-change path now clears the preview, error, and Apply action so the edited candidate must be compared again before applying. Vite browser smoke was attempted, but `http://127.0.0.1:1420/` was blocked by the local browser policy; no manual UI smoke is claimed for this row.

Latest Review Desk candidate tab-switch safety checks: 2026-06-02 code inspection found that a manual candidate preview was tied to candidate text, but not to the tab that produced the comparison. The preview now records the source tab id, Apply candidate is enabled only while that same tab is active, and the apply handler rejects stale tab ids. In-app browser smoke was blocked by the local browser policy for `http://127.0.0.1:1420/`; built-app launch via `open -n` returned the local `kLSNoExecutableErr` Launch Services blocker despite valid bundle metadata and codesign. Use the checklist below for fresh manual UI confirmation.
...
Latest Review Desk compare/apply information design checks: 2026-06-02 automated gates (`npm run typecheck`, `cargo fmt --manifest-path src-tauri/Cargo.toml -- --check`, `cargo test --manifest-path src-tauri/Cargo.toml`, `npm run build:vite`, `npm run build`, `git diff --check`) passed after adding a preview meta row (Target file / Buffer at compare / Candidate size / Compared at HH:MM:SS), an "Apply will mark unsaved" caption under the Apply button, a stale preview banner with a Re-run Compare action that appears when the active tab is closed, the active tab is switched to another file, or the active buffer contents change after Compare, and a dimmed table for stale previews. The stale banner disables the Apply button until the user re-runs Compare, and the preview is now keyed by `comparedAt` and stale status in addition to the existing `documentTabId` / `documentContents` checks. Built-app smoke is pending in this automation environment; use the checklist below to confirm.

Latest Review Desk low-prominence entry checks: 2026-06-02 automated gates (`npm run typecheck`, `npm run build:vite`, `git diff --check`) passed after removing the persistent top-chrome Review Desk button. Review Desk remains reachable through `Cmd+Shift+R` / `Ctrl+Shift+R`, View > Review Desk, and `/review`; use the checklist below to confirm the lower-prominence entry behavior. No fresh built-app manual smoke is claimed for this row.

## Review Desk Manual Candidate Review

Run on the built app from `src-tauri/target/release/bundle/macos/hazakura editor.app` after `npm run build`, or on the Vite dev server. The Review Desk manual candidate MVP adds a paste area, diff preview, and explicit apply-to-buffer action inside the existing Review Desk surface; it does not move the right-pane compare route and it does not save files automatically. The v0.8 foundation slice promotes the manual-candidate input from a plain `<textarea>` to a Markdown-aware CodeMirror editor, but keeps Compare / Apply explicit and keeps every safety guard from the v0.7 MVP. The v0.8 information design slice adds a preview meta row (Target / Buffer at compare / Candidate size / Compared at HH:MM:SS), an "Apply will mark unsaved" caption, and a stale preview banner with a Re-run Compare action so the user can see exactly what is being applied and when the preview is no longer accurate.
...
3. Confirm Review Desk is not shown as a persistent top-chrome button. Open it with `Cmd+Shift+R` (or `Ctrl+Shift+R` on non-mac), then close and reopen it from View > Review Desk. Confirm the Review Desk surface opens with the existing empty-state card plus a new "Manual candidate text" input slot (label, hint, Markdown-aware editor with line numbers, Compare, Clear).
...
6. Confirm the preview meta row directly under the preview header shows the target file label, "Buffer at compare" with line / char counts, "Candidate size" with line / char counts, and "Compared at" as a HH:MM:SS 24h timestamp close to the current local time. Confirm the "Apply will mark unsaved" caption appears under the Apply button so the user knows the file stays unsaved until they save it manually.
7. Confirm Apply candidate is enabled only after the candidate preview exists, then press Apply candidate. Confirm the editor buffer is replaced by the candidate text, Review Desk remains open, the candidate editor / preview / error are cleared, and the status bar reports success.
8. Confirm the file was not saved automatically: the active tab is dirty after Apply, and the on-disk file remains unchanged until Save.
9. Save the active tab and confirm the normal save flow persists the applied candidate text.
10. Edit the candidate editor again (e.g. add a heading, indent a list, or paste more Markdown) and press Compare again to confirm the diff preview updates to the new content, the syntax highlighting re-renders, and the meta row's "Compared at" timestamp updates to the new compare time.
11. After a successful Compare, switch to another open tab from the top tab bar and confirm the old preview is no longer applicable: a stale preview banner with a "Re-run Compare" action appears, the diff table is dimmed, the active tab is named in the reason copy, and Apply candidate is disabled. Press Re-run Compare and confirm the banner clears, the table restores, and Apply candidate is re-enabled.
12. After a successful Compare, edit the active editor buffer directly and confirm the same stale behavior: a stale preview banner explains the buffer was edited, Re-run Compare appears, the table is dimmed, and Apply candidate is disabled. Press Re-run Compare and confirm the diff is rebuilt from the new buffer.
13. After a successful Compare, close the compared tab (with the tab close button or `Cmd+W`) and confirm the stale preview banner shows the "compared tab is no longer open" reason, the table is dimmed, and Apply candidate is disabled. Reopen or create a text tab, press Re-run Compare if it is available, and confirm the preview rebuilds for the active tab.
14. Press Clear and confirm the candidate editor, the diff preview, the meta row, the apply caption, and the stale banner are all reset to the empty state.
15. Close the Review Desk with the close button, reopen it with `Cmd+Shift+R`, and confirm the previous candidate input, diff, meta row, and stale banner are gone (close-button reset path).
16. Re-enter a candidate, run Compare, close Review Desk with `Cmd+Shift+R` / `Ctrl+Shift+R`, reopen it, and confirm the candidate input, diff preview, meta row, apply caption, stale banner, and error state are gone (shortcut reset path).
17. With Review Desk open and no editor tab active, confirm the candidate editor becomes read-only, Compare and Apply candidate are disabled, and a "No editor tab is open to compare" note is visible.
18. Confirm Tab / Shift+Tab in the candidate editor still indent and outdent the line / selection without leaving the editor, and that Cmd+Shift+Enter does not send anything to an Agent session from inside the candidate editor.
19. Confirm the right-pane compare route is unchanged: open Diff in the top editor chrome, pick a source and target file, run Compare, and confirm the right pane shows the existing file comparison view (not the candidate preview).
20. Confirm the existing buffer-vs-disk / draft-vs-disk review paths still work through the right pane and do not leak into the Review Desk surface.
21. Confirm the Japanese / English menu language switch in Preferences still localizes the new Review Desk copy (input label, hint, placeholder, Compare / Apply candidate / Clear, empty heading, preview title, meta row labels, "Apply will mark unsaved" caption, stale banner heading, stale reason, Re-run Compare action).
22. In the editor, type `/review`, use `ArrowUp` / `ArrowDown` if needed, press `Enter`, and confirm the Review Desk opens without inserting a stray newline.
23. In the editor, type `/shortcut`, press `Enter` or `Tab`, and confirm a Markdown shortcut list is inserted into the buffer.
...
2. Open one or more throwaway Markdown files and confirm tabs, preview, outline, diff, and agent toggles in the top editor chrome still work, and that Review Desk is not present as a persistent top-chrome button.
3. Press `Cmd+Shift+R` (or `Ctrl+Shift+R` on non-mac) and confirm the editor area is replaced by a Review Desk surface with the title `Review Desk`, an English / Japanese localized empty-state card, and current manual-candidate guidance.
