...
Latest v0.7 release-prep rerun: 2026-06-02 managed-environment gates passed for `npm ci`, `npm run typecheck`, `cargo fmt --manifest-path src-tauri/Cargo.toml -- --check`, `cargo test --manifest-path src-tauri/Cargo.toml`, `npm run build:vite`, `npm run build`, `npm audit`, `cargo audit --file src-tauri/Cargo.lock` with temporary `CARGO_HOME`, and `git diff --check`. Fresh built-app UI smoke was not claimed: `open -n "src-tauri/target/release/bundle/macos/hazakura editor.app"` returned `kLSNoExecutableErr`, Computer Use denied access to `lab.hazakura.note`, and `npm run build:dmg-preview` stopped at `hdiutil: create failed - 装置が構成されていません`. Re-run built-app smoke and warning-expected DMG creation in a host session with Launch Services, Computer Use, and `hdiutil` available before tagging v0.7.0.

Latest Change Review routing checks: 2026-06-02 user smoke found that the top-right `変更を確認` dirty-buffer button reported readiness but did not reveal the comparison. Automated gates passed after routing successful buffer-vs-disk and draft-vs-disk reviews to the existing right-pane Diff surface: `npm run typecheck`, `cargo fmt --manifest-path src-tauri/Cargo.toml -- --check`, `cargo test --manifest-path src-tauri/Cargo.toml`, `npm run build`, and `git diff --check`. Re-smoke on the built app by editing a saved Markdown file, pressing `変更を確認`, and confirming the right-pane `変更確認` view appears.
