...
v0.15.0 records a user-test polish checkpoint and publishes a warning-expected macOS DMG preview for other Macs. The focus is small issues found by using the app: stale-state prevention, L Mode lightness, Hazakura Local Assist rough-request quality, settings/theme clarity, and safe save/recovery behavior.
...
- Prevents stale async completions from overwriting newer compare, disk-change review, image preview, Hazakura Local Assist candidate, HTML export, Save As, and recovery-reopen state.
...
### Hazakura Local Assist And Settings Polish

- Separates rough user requests from internal Hazakura Local Assist instructions so internal prompt text does not leak into the visible request payload.
- Adds a focused instruction helper for rough writing requests and guards stale Hazakura Local Assist review candidates.
- Improves Hazakura Local Assist window affordances around document refresh and target acquisition failure.
