...
if (location.hostname !== "localhost" || location.port !== "1420") throw new Error("Use isolated browser QA origin localhost:1420");
window.localStorage.setItem("hazakura-note-theme", params.get("theme") ?? "light");
// 編集クロームQA: 実アプリのAssistラベルだけを確認（native呼び出しは下でスタブ）。
if (params.get("assist") === "apple-local") window.localStorage.setItem("hazakura-note-assist-surface", "apple-local");
window.localStorage.setItem("hazakura-note-editor-settings", JSON.stringify({lModeEnabled: params.get("view") === "lmode"}));
