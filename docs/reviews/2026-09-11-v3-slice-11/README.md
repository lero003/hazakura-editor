...
### P2-low — 草稿が「一度の書き出し操作」を越えて残っていた

`useExportDrafts` は説明（**一度の操作のあいだだけ**保持）に反して clear を持たず、文書が変わった時だけ無効化していた。
そのため同じ文書で `EPUB を開く → 本全体＋書名 → キャンセル → 後で書き出しを開始` とすると、
**前回の `scope=book` と入力が残ったまま**始まっていた。

`clear()` を追加し、**利用者のキャンセル／確定**で捨てる（形式切替の内部キャンセルでは捨てない）。
`AppOverlays` の EPUB / PDF / HTML の `onCancel` / `onConfirm` に配線した。

回帰テスト（`ExportFormatRoundTrip.test.tsx`）:「書名＋本全体 → キャンセル → 改めて開始」で
**書名が空・対象が「現在のファイル」**に戻る。`clear()` を外すと落ちることを確認（TEMP）。

### P3 — 古い `null === null` テストを削除

前回は正しい検査を**追加**しただけで、壊れた比較（両側とも `div[role='dialog']` を探す）が残っていた。
古いテスト1本（33行）を削除し、実在＋包含＋単一性の検査だけにした（`ExportDialogFrame.test.tsx` は 7 → 6件）。
