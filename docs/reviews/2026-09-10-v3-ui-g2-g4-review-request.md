...
Scope: `fa6c9ba0..2a118313`（codex/v3、8コミット）
Authority: Request
Date: 2026-09-10
...
| **[P2] G3: コントラストテストが実CSSを読まず、nav面が未検査** | テストは `--text` / `--text-muted` / `--nav-surface` を**CSSから実取得**する形に変更（値を書き写さない）。不透明ナビ5テーマの自動検査を追加。半透明の edohigan / shinkai は「`rgba()` 宣言であること」だけを固定し、コントラストは**実描画ピクセル測定**（`rgb(47,30,45)` / `rgb(11,40,54)` に対し 12.67・6.00 / 12.46・5.96）へ分離し、資料の断定も「合成条件付き」に弱めた |
