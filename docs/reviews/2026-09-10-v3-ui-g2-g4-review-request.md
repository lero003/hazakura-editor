...
Status: Review request（7回目）
Implementation scope: `fa6c9ba0..7a197933`（19コミット: G2〜G4・段階2・第二調整・手直し3点・focus/境界・設定外枠）
Review packet: この依頼文を含む docs-only コミット（`7a197933` の直後1コミット。`fa6c9ba0..` はブランチ全体で20コミット）
再現: `git log --oneline -20`
Authority: Request
Date: 2026-09-10
...
前回の依頼文で「4コミット」と書いていたのは誤り（`c937e036` が抜けていた）。現在は17コミットで、
...
| 指摘 | 対応 |
| --- | --- |
| **[P2] focus 3:1の検査が実ringを見ていない** | 意味トークン `--focus-ring`（既定 `var(--accent)`）を新設し、薄めた9箇所（56%/60%/28%/35%/50%/55%/70%）を**すべて薄めない実値**へ。light 2.40:1→**5.85:1**、shokou 2.11:1→**4.30:1**、タブClose 2.59/2.24→同値。`focusRingCss.test.ts`（新規）が**全CSSで `outline` に color-mix を使わないこと**を固定し、`themeContrast.test.ts` は `--focus-ring` の実値で3:1を検査 |
...
- ローカル：`npm run typecheck` 成功、`npm test` **262ファイル / 2,316件**、`smoke:app-store-surface` 117件、
...
- 画面23（狭い窓の一時折畳み）／画面16（書体・行間は別タスク）／画面24（寸法差の計測）
