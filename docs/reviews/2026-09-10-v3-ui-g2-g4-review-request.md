...
Status: Review request（6回目）
Implementation scope: `fa6c9ba0..d7fa1cd0`（17コミット: G2〜G4・段階2・第二調整・手直し3点・focus/境界）
Review packet: この依頼文を含む docs-only コミット（`d7fa1cd0` の直後1コミット。`fa6c9ba0..` はブランチ全体で18コミット）
再現: `git log --oneline -18`
Authority: Request
Date: 2026-09-10
...
- ローカル：`npm run typecheck` 成功、`npm test` **261ファイル / 2,314件**、`smoke:app-store-surface` 117件、
...
- 設定の外枠寸法（1100px参考・レール200px）とHelp導線、画面16/05/23/24
