...
Status: 修正済み（ブラウザ検証済み / WKWebView実機受入済み 2026-09-18）
...
### 追補 — 外部レビュー R2 / R3 への対応（2026-09-18）

固定 SHA `281a4863` へのレビューで、末尾補正の判定漏れと、操作切替時の取りこぼしを指摘された。

**R2: つまみ中央を握ると補正に入らない。** 判定がポインタの絶対位置
（`clientY >= rect.bottom - 4`）だったため、つまみの中央をつかんで末尾まで運ぶと
（ポインタはつまみ半分ぶん上で止まる）補正が起動しなかった。ドラッグ開始時に
「つまみが下端へ届くポインタ Y」を計算する方式へ変えた。

再実測（1440×900・常時表示スクロールバー・トラック 698px / つまみ 184px）:

| 操作 | 結果 |
|---|---|
| つまみ中央をつかみ、つまみ下端がトラック下端へ届く位置で離す | `scrollTop = 2660 = max`（不足 0px） |
| トラック中央で離す | `scrollTop = 649`（不足 1656px・勝手に寄せない） |
| ポインタをトラック端より下へ動かして離す | `scrollTop = 2660 = max`（不足 0px） |

**R3: ガード中の操作切替で最後のスクロールを捨てる。** 同期ガード中の反対ペインの
scroll を一律に捨てていた。ホイール / ポインタ / キーの入口でその向きの所有権を
手放すようにし、編集側の延長は「同期先への書き込み差分 10px 以上」から切り離した。
再実測: 編集のドラッグで末尾へ寄せた直後にプレビューをホイールで上へ動かすと、
両ペインが同じ比率（0.652）へ揃う（切り離し前は本文が末尾に残った）。

回帰テスト: `EditorPane.test.tsx`（つまみ中央・端越え・中央離し・複数フレームの伸び）、
`usePreviewScrollSync.test.ts`（操作の受け渡し 2 方向・差分 10px 未満でも延長）。

### 追補2 — 再レビュー R2 / R3 の残条件（2026-09-18）

固定 SHA `8f560695` への再レビューで、幾何モデルの反例と、予約済み rAF の競合を指摘された。

**R2: 横スクロールバー併設で縦トラックを過大に見積もる。** `rect.height` / `rect.bottom` を
縦トラックとして使っていたため、横バーがレイアウト領域を占めると（macOS の「常に表示」）、
つまみ長と下端到達 Y がずれた。横向きの帯（`offsetHeight - clientHeight`、オーバーレイでは 0）を
除いた縦トラックで計算するようにした。

幾何モデル（表示領域 500 / 本文 2000 / 横バー 15 / つまみ 125）での回帰テスト:

| 操作 | 結果 |
|---|---|
| つまみ中央（y=62.5）をつかみ、真の末尾到達 Y=437.5 で離す | 再計測後も末尾（不足 0px） |
| 同じ条件で 420px（途中）で離す | 補正しない（その位置で停止） |

横バーを無視する旧式では、末尾到達 Y を 448.75 と計算するため 437.5 の操作を取りこぼす
（この回帰テストは修正前実装で落ちることを確認）。

**R3: 予約済み rAF が所有権を奪い返す。** ガード解除が timer/source だけで、予約済みの
同期 rAF を破棄していなかった。解除時にその向きの rAF を取り消し、世代番号で古い通知を
失効させる（`previewSyncGenerationRef` / `editorSyncGenerationRef`）。
回帰テストは「旧予約を残したまま反対ペインを操作 → 旧 rAF を後から実行 → 新しい位置が勝つ」。
修正前実装（世代チェックなし）で落ちることを確認した。

プレビュー側の入力入口には `keydown` を追加した（PageDown / PageUp / Space / 矢印 / Home / End。
...
- 実機での受入は済み（2026-09-18、オーナー報告・問題なし）。「常に表示」と
  オーバーレイのどちらで確認したかの内訳と、重なり幅の実測値は記録していない。
...
実機の追加報告: 編集面のスクロールバーをドラッグしたとき、**キャレットのある行へ
吸い寄せられる**ことがある（プレビュー側では起きない）。

### 調べたこと（in-app browser / Chromium では再現せず）

同じ fixture で、キャレットを実クリックで置いてからつまみをネイティブドラッグしても、
`.cm-scroller` への JS 書き込みは **0 件**（`?traceScroll=1` の記録が空）で、離した
位置のまま止まった。キャレットを画面外へ運ぶ場合・キャレットの近くで離す場合・
最上端まで引く場合のいずれも同じ。オーバーレイ（`?scrollbar=classic` なし）では
そもそも右端 4px のドラッグが本文の選択になり、つまみを掴めなかった。

書き換え経路はコード上 2 つに絞られた。どちらも**こちらが mouseup でフォーカスを
戻す**（v0.34 の「ドラッグ中は blur、離したら復帰」）ことで初めて動く。

1. CodeMirror `observers.focus` の "When focusing reset the scroll position"。
   `scrollTop` が 0 のとき、直前に観測した位置へ戻す。`view.focus()` から同期で走る
   （jsdom では CodeMirror 自身の `preventScroll` 代替処理が同じ同期で戻すため、
   ユニットテストでは見えない）。
2. Safari 26 系では `focus({ preventScroll: true })` が効かず、フォーカス時に
   キャレット位置までスクロールする（CodeMirror 側も Safari 26 では `preventScroll` を
   無効化して自前で戻す実装になっている）。

### 直した内容

`EditorPane` の mouseup 処理で、フォーカスを戻す直前にドラッグが決めた位置を控え、
復帰後に**その 1 フレームだけ**取り戻す（`holdDraggedScrollTop`）。ユーザー自身が
新しくホイール・クリック・キー入力したら、その時点で降りる。末尾へ寄せ直す処理
（前節）とは役割を分けてあり、トラック下端で終わったドラッグの扱いは変えていない。

### 検証

`EditorPane.test.tsx` の新テスト2件。保持の呼び出しを外すと
「フォーカス復帰でキャレット側へ引き戻されても、ドラッグ位置を保つ」が落ちる
（`expected 500 to be 300`）。「直後にユーザーが操作したら上書きしない」も固定した。

実機受入: オーナーが WKWebView で確認し「本文の途中にカーソルを置いたまま、離れた
位置までドラッグして離す」を含めて問題なしと報告（2026-09-18）。in-app browser は
Chromium なので、フォーカス時のキャレット移動そのものはブラウザ側では再現できない。
