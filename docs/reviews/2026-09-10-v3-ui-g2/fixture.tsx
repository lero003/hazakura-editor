...
// UI-G1と同じ構成。設定カテゴリレール（dialogs.css の min-width:800px / min-height:481px）を確認する。
...
function Fixture() {
  // テーマは確認したいものを ?theme=dark のように指定する（既定は light）。
  const requestedTheme = (new URLSearchParams(window.location.search).get("theme") ?? "light") as ThemePreference;
  ...
  const [theme, setTheme] = useState<ThemePreference>(requestedTheme);
