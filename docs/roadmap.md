...
| ソース版 | 3.0.1。codex/v3でUI-G1〜G4（状態表示・Help、設定レール/現在地、紙面/ナビ面トークン、開始画面2ペイン）と段階2（モック配色・chrome面トークン）・第二調整（罫線の階層とfocus/native契約の検査）・設定外枠・狭幅23、続いてモック起点の再点検（江戸彼岸の明色化・スライスC/D/E・07本文面化・11形式ナビ）とレビュー修正（R1〜R5・P1/P2）を実装し、v3.0.0として公開。 |
