...
- Current package/app version: `1.0.0` release candidate.
- Latest source / local-app tag: `v0.35.0`.
- Mac App Store published version: `0.36.0`, carrying the completed v1
...
- Latest local App Store / TestFlight package candidate metadata
  ...
  `1.0.0` candidate has passed the local release gates; upload,
  TestFlight distribution, App Review, and publication remain separate.
- Active lane: `1.0.0` proof-close and product communication, presenting the
  shipped Safe Editor, L Mode, e-book Mode, EPUB export, Diff / Recovery,
  and Hazakura Local Assist review surfaces as one coherent
  single-document book-writing product without adding a new surface.
