...
- 2026-05-28のworkspace image test polishでは、PNG/JPEG/GIF/WebPの対応signature、拡張子不一致拒否、20 MB超過拒否をRust回帰テストで固定済み
...
- 2026-05-28のlocal bundle minimum-system polishでは、生成 `.app` の `LSMinimumSystemVersion` をRust実行ファイルの `minos 11.0` と合わせるように修正済み
