...
Authority: Medium
Last reviewed: 2026-07-02 (observation-driven backlog refreshed: theme list corrected, settings surface consistency added)
...
- Current package/app version: `1.3.0` candidate baseline (v1.3 Daily
  Trust). The published Mac App Store version remains `1.1.0`.
- Latest source / local-app tag: `v0.35.0`.
- Mac App Store published version: `1.1.0`, carrying the completed v1 feature
  shape plus per-document Editor / Reader / Preview position continuity.
- Latest local App Store / TestFlight package candidate metadata
  (version, build counter, pkg path, SHA-256, generated time) lives in
  `docs/internal/app-store-candidates/latest.json`, regenerated by
  `npm run release:candidate -- --with-app-store-pkg`. A signed local
  `1.2.0` candidate passed the local release gates; the `1.3.0` signed pkg
  has been built, uploaded to App Store Connect, and submitted to
  TestFlight / App Review. Apple processing / review outcome and public
  release remain pending; raw App Store Connect, TestFlight, and App
  Review logs are not tracked in this repository.
- Active release lane: v1.3 Daily Trust. Preserve the editing session through
...
11. v1.3 Daily Trust is implemented and the `1.3.0` candidate is in
   TestFlight / App Review: Save As session continuity, explicit Local
   Assist `採用` / `破棄`, richer Reading Focus TOC context, and fixed A4
   PDF margin presets. Package metadata reads `1.3.0`; the signed pkg has
   been built, uploaded to App Store Connect, and submitted, but Apple
   processing / review outcome and public publication remain pending.
   Full gates and representative built-app interaction/rendered-PDF proof
   pass. Extended RC interaction breadth remains in `docs/v1.3-followup.md`.
