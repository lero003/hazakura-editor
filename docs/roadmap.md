...
| Core AI models | Allowlisted `.aimodel` catalog + ローカル/外部ソース（C-3） | allowlist配布は実装済みで、identity / asset pack / manifest / AOT / bake-off後に本番entryを公開。C-3 は app-managed Custom Models の検出・一覧・選択・helper生成経路まで接続し、外部登録 / bookmark と実モデル受入は未完了 — `core-ai-model-source-abstraction.md` |
...
**v3.1 追加レーン（オーナー決定 2026-09-22、app-managed 選択接続まで実装）:** Apple-hosted 以外のモデルソース
...
`docs/core-ai-model-source-abstraction.md` に固定した。Rust / Swift のローカル契約に加え、
app-managed Custom Models は既存 registry / UI / helper の選択・生成経路へ接続した。外部登録、
security-scoped bookmark、実 local model の受入は未完了。
C-1 / C-2 を止めず、
