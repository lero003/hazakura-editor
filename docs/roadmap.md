...
| Core AI models | Allowlisted `.aimodel` catalog + ローカル/外部ソース（C-3） | allowlist配布は実装済みで、identity / asset pack / manifest / AOT / bake-off後に本番entryを公開。C-3 は app-managed Custom Models の検出・一覧表示まで実装し、選択・生成・外部登録は未接続 — `core-ai-model-source-abstraction.md` |
...
**v3.1 追加レーン（オーナー決定 2026-09-22、検出・一覧表示まで実装）:** Apple-hosted 以外のモデルソース
...
app-managed Custom Models は既存 registry / UI へ検出表示まで接続した。選択・生成と外部登録は未接続。
C-1 / C-2 を止めず、
