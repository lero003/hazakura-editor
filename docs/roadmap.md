...
- Latest local App Store / TestFlight package candidate metadata
  ...
  `1.2.0` candidate passed the local release gates; the `1.3.0` candidate
  has aligned package metadata but its signed pkg has not been built yet.
  App Store Connect upload, TestFlight, and App Review remain outside this
  repository until the user explicitly opens that lane.
- Active release lane: v1.3 Daily Trust. Preserve the editing session through
  ...
  new surface or entering v2 Book Scope. Tagging, upload, and publication
...
11. v1.3 Daily Trust is implemented and is the prepared `1.3.0` candidate:
   Save As session continuity, explicit Local Assist `採用` / `破棄`, richer
   Reading Focus TOC context, and fixed A4 PDF margin presets. Package
   metadata now reads `1.3.0`, but the signed pkg build, upload, and
