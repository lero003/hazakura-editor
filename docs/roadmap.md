...
- Current package/app version: `1.0.0` released baseline.
- Latest source / local-app tag: `v0.35.0`.
- Mac App Store published version: `1.0.0`, carrying the completed v1
  feature shape: Editor / Reader Position Bridge, direct PDF and EPUB
  export, e-book page-turn stabilization, workspace markers, Markdown
  insertion helpers, and Hazakura Local Assist as a preview on-device
  writing companion.
- Latest local App Store / TestFlight package candidate metadata
  ...
  `1.0.0` candidate passed the local release gates before publication.
  Public version `1.0.0` is confirmed by the listing; raw App Store
  Connect, TestFlight, App Review, and public build-number logs remain
  outside this repository.
- Active lane: v1.1 Daily-use Continuity And Trust. Improve position,
  recovery, image, and review confidence one reproduced issue at a time
  without adding a new surface or entering v2 Book Scope.
...
9. v1.1 prioritizes daily-use continuity and trust: position ownership
   across editor / Preview / reader, then Recovery reliability, followed
   by one-at-a-time image and Diff review edge cases.
10. v1.2 handles expectation-setting and lower-risk polish around Local
   Assist quality, export UX, safe command discovery, and empty states.
...
preserved in `docs/v1.1-v1.2-followup.md` and now feed the active v1.1
