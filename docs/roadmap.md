...
- Latest local App Store / TestFlight package candidate metadata
  ...
  `npm run release:candidate -- --with-app-store-pkg`. The prior
  `0.34.0` candidate is superseded for PDF output because TestFlight
  still showed macOS' "This application does not support printing"
  alert.
