...
2026-09-22のオーナー決定で、v3.1にはこれに加えてローカル/外部のモデルソース（C-3、
`core-ai-model-source-abstraction.md`）も含める。v3.0の範囲は変えない。
...
C-3（ローカル/外部モデルソース、2026-09-22オーナー決定）も同じRust単一registryを拡張して扱い、
別registryを作らない。
