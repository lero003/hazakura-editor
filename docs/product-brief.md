...
[Assist Surface Strategy](assist-surface-strategy.md) は、既存のExternal Agent Workbenchを分離可能な境界に置き、Apple Foundation ModelsベースのApple Local Assistへ切り替えられるようにするための設計方針である。App Store build / developer build の分離方針は [Apple Local Assist And Distribution Plan](apple-local-assist-distribution-plan.md) に置く。Apple Local Assist の体験像は [Apple Local Assist Writing Companion Plan](apple-local-assist-writing-companion-plan.md) を優先する。

ただし、これは汎用AI platform、provider plugin、任意コマンド実行、agent自動適用への承認ではない。Apple Local Assistを入れる場合も、L Mode / Safe Editor での執筆を補助する外出し Writing Companion として扱い、AI が本文を直す場合は明示的なユーザー依頼、AI edit transaction、Diff / change history での確認、no auto-save を守る。
