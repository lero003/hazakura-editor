...
ただし、これは汎用AI platform、provider plugin、任意コマンド実行、agent自動適用への承認ではない。Hazakura Local Assistを入れる場合も、L Mode / Safe Editor での執筆を補助する外出し Writing Companion として扱い、AI が本文を直す場合は明示的なユーザー依頼、AI edit transaction、Diff / change history での確認、no auto-save を守る。
