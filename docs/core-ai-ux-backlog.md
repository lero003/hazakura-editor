...
- いまは Settings 内の 1 ペイン。**ダウンロード管理として独立した画面にしたい**:
  ダウンロード（開始 / 進捗 / 再開 / 取消）、保存先の説明、削除、サイズ、バージョン、
  ライセンス表示を1か所にまとめる
- 画面の入口は Settings から独立させてもよいが、**自動ダウンロードや起動時スキャンは足さない**
  （既存の「明示操作」境界を維持する）
- モデルごとに状態（未取得 / 取得中 / 検証済み Ready / 失敗）と操作を並べ、選択中モデルの
  切替（System 含む）と削除時の System 退避も同じ画面で完結させる
