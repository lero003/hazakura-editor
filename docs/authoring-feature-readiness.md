...
| --- | --- | --- | --- |
| Image paste | Clipboard image paste can call `save_pasted_image`, write supported image bytes into workspace `assets/`, reuse the existing hash-named asset for duplicate pasted-image bytes, and insert Markdown. Workspace-relative images under the selected workspace, including pasted `assets/...` references and README-style `docs/images/...` screenshots, render in preview/export/print through the existing workspace-image validation path and are inlined as data: URIs. | Alt text, visible error/status, and release-facing smoke coverage are incomplete. | Add smoke coverage for the image paste + preview + export flow. |
| Drag & drop image ingestion | Images dropped on the app window are imported into workspace `assets/` via `import_image_from_path` command, Markdown is inserted at the editor cursor. | Smoke coverage missing. Handles PNG/JPEG/GIF/WebP. Text-file drag-drop already existed. | Add smoke path for image drag-drop. |
| PDF / HTML export | File menu entries exist. HTML export writes standalone HTML through a save dialog (CSS variables from current theme, pasted images inlined). Print to PDF opens a standalone rendered Markdown HTML document for system/browser printing instead of printing the full editor UI. | CSS parity with live preview is partial (no full theme import). PDF has no explicit app-level output path and depends on the system/browser print dialog. Smoke checklist coverage is missing. | Tighten HTML export parity with preview, then document PDF as "Print to PDF" unless a real PDF pipeline is chosen. |
| Ambient effects | Seasonal themes now use theme-specific CodeMirror colors and adjustable ambient intensity. | This is visual polish only; it must not replace editor navigation, preview, or review workflows. | Keep Off / Subtle / Normal / Dramatic smoke coverage with reduced-motion checks. |
| Spellcheck | CodeMirror content gets `spellcheck=true/false` through the View menu toggle (Cmd+Option+;). | This is WebView/browser spellcheck wiring, not a macOS `NSSpellChecker` command path. Suggestions, language control, and context menu are not implemented. Smoke coverage is missing. | Decide the MVP is WebView spellcheck, document that boundary, and add a smoke path. |
| Table editing | Toolbar button and shortcut (Cmd+Shift+T) can insert a fixed 3-column Markdown table. Preview table styling exists. | There is no WYSIWYG-like table editor, right-click row/column add/delete, or alignment editing. | Treat current behavior as "Insert table" only; add row/column/alignment commands in later narrow slices if still desired. |
| Agent Workbench authoring actions | Agent Workbench can run one allowlisted provider, can send a workspace file path to the active provider. Terminal resize is now debounced (100ms). Diff/review exists separately. | There are no selection-range actions for summarize/proofread/translate, no captured candidate result, and no selection-to-diff apply flow. | Start with a context-helper only: send selected text plus an explicit instruction to the running Agent session, without auto-apply. True inline assistance needs a separate diff/apply design review first. |

## Recommended Order

1. ✅ Complete image paste by making pasted `assets/...` references visible in preview and HTML export without opening arbitrary local images. *(Done)*
2. ✅ Add drag-and-drop image ingestion into `assets/` using the same backend validation and Markdown insertion path as clipboard paste. *(Done)*
3. Add smoke coverage for image paste, image drag/drop, HTML export, Print to PDF, ambient effects, spellcheck toggle, and table insertion. *(Remaining)*
...
Choose exactly one small slice from docs/authoring-feature-readiness.md, in this order: safe workspace-relative assets image rendering in preview/export; image drag-and-drop into assets; export smoke/parity; ambient/spellcheck smoke and docs; table insertion honesty; Agent selected-text context-helper design. Do not implement multiple areas in one run.
