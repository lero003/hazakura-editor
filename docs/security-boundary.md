...
- 外部画像を勝手に読み込まない（リモート既定オフ、https のみ・有界 fetch・redirect 追跡なし）
- ローカルファイル参照の扱いをUI上で明示する
- ブロックした画像は、理由（workspace 外相対 / 絶対 / リモート / 未配置など）と
  次の操作をプレースホルダに書く
- workspace 外ローカル画像は、既定の **現在の文書で親フォルダを明示許可**、または
  警告付きの **すべて読む（OS が許可した範囲）** 設定のときだけ読み込む。文書単位の
  許可は新しく開いたタブへ引き継がず、localStorageへ保存しない。Rust は canonical path、
  image型、サイズを検査し、既定モードでは承認 root 外を拒否する
...
- Preview のリモート読込がオフのとき、上記固定コマンドが `https` 画像へ通信する前に
  警告確認を行う。拒否時は通信・コピー・source 編集を行わない
