...
- Local App Store / TestFlight package candidate metadata (version,
  ...
  for the active artifact. At this sync point it records `0.36.0` build
  `50` with `smoke: skipped`. The prior native-print candidate is
...
- `0.36.0` is now the source/package app version locally. The latest
  local App Store / TestFlight candidate metadata records build `50`.
  ...
  `docs/internal/app-store-candidates/latest.json` as the single source
  of truth. The remaining proof is user-side TestFlight install / launch,
  confirming long illustrated documents page without skipping, and
  confirming exported EPUB page-breaks in an actual reader.
