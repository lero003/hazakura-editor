...
- Local App Store / TestFlight package candidate metadata (version,
  ...
  longer carry per-build SHA / pkg path / build counter values; consult
  `latest.json` instead. The prior candidate is superseded for PDF
  output: local manual native-print smoke passed, but the TestFlight
  build still showed macOS' "This application does not support printing"
  alert. Do not revive the user-facing macOS print UI path as the fix.
