...
- v1.10 以降の単一文書構造化で、heading / frontmatter / page-break /
  navigation の shared structure model と明示的なsingle-buffer編集を実証している。
- v1.11 の明示的・bounded・read-onlyなOKF v0.1 Draft compatibility preview
  と配布版総合テストが完了し、既知の残リスクを記録している。
- v1.3 以降のリリースレーンが安定している。
- 本ドキュメントの UI 方針が、少なくとも一つの候補へ収束している。
- OKF v0.1 Draftの互換範囲と、OKFが定義しないHazakura固有のBook semanticsを
  分離できている。
...
1. v1.8 Daily Trust と v1.9 Writing Loop Clarity を通じて、複数ファイルを
   扱う前の耐久性・観測性・利用目的の足場を確認する。
2. v1.10 以降で、一つの Markdown source 内の章・節・見出し・目次候補を
   shared structure model として扱い、Undo可能な明示編集を実証する。
3. v1.11でOKF v0.1 Draft compatibility previewを明示的・bounded・read-onlyに
   実装し、Book semanticsを持ち込まずにconcept / reserved file / linkを確認する。
4. 構造化とOKF previewを含む配布版総合テストを行い、長文、復旧、Reference、
   Assist、export、a11y、sandbox、failure boundary を横断確認する。
5. v2着手時にOKFの最新版を再確認し、base compatibilityとHazakura固有拡張を
   分離した実装契約を決める。
6. Open UI Questions の各問いについて、UI Candidates の候補を比較し、
   少なくとも一つへ収束させる。
7. 収束した UI 方針で、v1 の AppWorkspace / view-state registry / e-book
   Mode を構造へ拡張する最小スライスを設計する。
8. 本ドキュメントの Status を Proposal から Review へ上げ、実装契約として
   確定できるか判断する。

`v1.10` は v2 の番号を先取りするものではない。単一 source 内の構造解釈と
編集を証明する橋渡しであり、複数ファイル章順、Book manifest、whole-book
scope は本ドキュメントの v2 境界に残す。詳細は
`docs/v1.8-plus-product-review-roadmap.md` を参照する。
