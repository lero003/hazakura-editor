...
Last reviewed: 2026-07-11 (v1.10 single-document structure bridge accepted)
...
- その構造化機能を含む v1.11+ 配布版総合テストが完了し、既知の残リスクを
  記録している。
...
3. 構造化を含む v1.11+ 配布版総合テストを行い、長文、復旧、Reference、
   Assist、export、a11y、sandbox、failure boundary を横断確認する。
4. OKF の最新仕様を再確認し、構造型 Markdown の構造的基盤として採用
   できるか判断する。
5. Open UI Questions の各問いについて、UI Candidates の候補を比較し、
   少なくとも一つへ収束させる。
6. 収束した UI 方針で、v1 の AppWorkspace / view-state registry / e-book
   Mode を構造へ拡張する最小スライスを設計する。
7. 本ドキュメントの Status を Proposal から Review へ上げ、実装契約として
   確定できるか判断する。
