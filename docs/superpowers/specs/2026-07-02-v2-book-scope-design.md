...
Last reviewed: 2026-07-18 (nested index ordering + heavy manuscript sample)
...
### Heavy manuscript evidence and held items
...
## Settled Book View Presentation (UX quieting)

2026-07-18 の quieting スライスで、保存済み Book view の情報密度を次に固定する。

- **主操作**: 章数・「本全体を読む」・iconの「編集」・「その他」を1行に置く。
  portable recipe と問題時の再確認は「その他」に畳み、章立て済みの画面では候補生成を並べない。
- **候補を作る**: 空状態と章編集中の setup 操作。index.md 順の提案はここでだけ。
- **再確認**: 利用不能章があるときだけ「その他」に出す。問題がない日常ループでは出さない。
- **章行**: 保存済みgroup / document階層を静かなindentで表示する。ルート直下は
  ファイル名のみ、入れ子 path だけ副題、titleに相対path。名前と副題は改行せず省略し、
  上下/削除操作はhover/focus時だけ重ねて通常時の文字幅を奪わない。上下矢印は同じ親内だけ。
...
| 11 | Help expansion + other under-explained feature docs | **Done in source** — Help “Books and knowledge folders”; tree `2.0.0` |
