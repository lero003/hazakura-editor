...
- Post-v0.16 v0.17 **Distribution Prep / Release Polish** implementation work is closed on `main`; remaining Help/content polish, real-app smoke, Store-submission material, and official distribution work are deferred to v0.18 / submission-prep lanes.  One optional pre-tag v0.17 release-candidate polish request remains documented for Apple Local Assist operation feedback because it directly improves alpha smokeability without widening the product boundary.
...
- `docs/v0.17-apple-local-assist-operation-feedback-request.md`: optional pre-tag v0.17 release-candidate polish request for bounded Apple Local Assist operation feedback.
...
v0.16.0 local release gates and warning-expected DMG preview verification passed on 2026-06-08. DMG SHA-256: `39f63f42fc46c7e2d8659858f1a93127917be8e7a1836f594d885a44105e40fb`. Cargo test ran serially (`-- --test-threads=1`) to dodge default-thread-pool flakiness in the Agent Workbench real-runtime test; 243 tests passed. The DMG and SHA-256 file are written under `src-tauri/target/release/bundle/dmg/`.

v0.16.0 GitHub Release publication and remote verification:

- Published at `https://github.com/lero003/hazakura-editor/releases/tag/v0.16.0`.
- Remote assets were downloaded into a fresh `src-tauri/target/release/remote-verify-v0.16.0-*` directory and passed `shasum -c`, `hdiutil verify`, mounted app metadata (`0.16.0`, `lab.hazakura.note.dev`, `hazakura editor Dev`, `hazakura-editor`), mounted app `codesign --verify --deep --strict --verbose=2`, icon-resource hash comparison against `src-tauri/icons/icon.icns`, and mounted app launch-process observation.
...
An optional pre-tag Apple Local Assist polish slice is allowed if kept
within `docs/v0.17-apple-local-assist-operation-feedback-request.md`:
show compact, window-local operation feedback so real-app smoke can see
target acquisition, sending, local generation, applied, failed, and
reviewable states.  This must remain an ephemeral UI trail, not raw
Foundation Models prompts / responses, provider transcripts, model
reasoning, persistent logs, or Support Diagnostics data.
...
A small in-app disclosure route is now reachable from Help: the native Help menu includes `Local Data Disclosure...` / `ローカルデータの扱い...`, and the command palette exposes the same route as `help.localDataDisclosure`. Both open the local-data pane (`PreferencesDialogMode` now includes `"privacy"`). The pane is read-only prose that mirrors what `docs/security-boundary.md` and `docs/apple-local-assist-distribution-plan.md` already let the project claim: local-file-only document access, `.hazakura/backups/...` is a workspace-local auto-backup folder, the preview blocks external images and dangerous HTML / script / iframe / object / embed and only routes link clicks to workspace-relative text file opens inside the selected workspace (external scheme links and absolute paths are ignored with a status message), HTML export inlines workspace images as data URIs, Apple Local Assist is on-device / explicit / unsaved / diff-reviewable, the App Store build omits Agent Workbench / CLI agent / arbitrary command surface, and the app's code does not include `fetch` / `XHR` / `analytics` / `telemetry` / `crash reporting` libraries. The disclosure also keeps local-process wording scoped: Apple Local Assist may use the bundled helper when enabled and supported, the Developer / GitHub lane can launch an allowlisted Agent Workbench provider when that mode is enabled, and explicit OS handoff actions such as Finder reveal, print handoff, and Move to Trash may pass selected paths or temporary files to macOS utilities.

The wording deliberately avoids overclaim: it says what the app's code does and does not include, not what the user's OS or third-party files may do. It is an in-app technical disclosure, separate from the public-copy Privacy Policy draft. The disclosure is reachable in both distribution lanes through Help so App Review can find the local-data route without enabling Agent Workbench or opening Settings.

The slice covers `app-store-quality: privacy-local-data` from `docs/app-store-quality-agent-requests.md`. Out of scope: final public Privacy Policy URL publication, website work, App Store Connect metadata, analytics / crash reporting, and certificate / signing work.
...
4. If preparing the v0.17 release checkpoint, use `docs/source-release-checklist.md`, `docs/dmg-preview-checklist.md`, `docs/app-store-quality-closeout.md`, and `docs/app-store-current-work.md`; keep App Store / Developer / warning-expected DMG lanes separate and do not mix v0.18 Help / Store submission / official distribution tasks into the v0.17 tag.  If the optional Apple Local Assist operation-feedback slice is attempted before tagging, keep it inside `docs/v0.17-apple-local-assist-operation-feedback-request.md`.
