...
- Split export behavior into `src/hooks/useDocumentExport.ts` and persisted preferences/theme/Agent Workbench settings into `src/hooks/useAppPreferences.ts`, reducing App.tsx from 6039 to 5721 lines without adding Context/zustand or changing the Agent boundary.
