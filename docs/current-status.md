...
- Current development lane is post-v0.7 follow-up and v0.8 UX planning. v0.7 shipped as a Review Desk MVP, not as the full editable two-column Review Desk workbench.
- Current automation emphasis should move from v0.7 release-prep to post-release triage, focused quality checks, and v0.8 planning docs unless the user explicitly opens a patch-release lane.
...
- Latest v0.8 Review Desk candidate editor foundation slice replaced the manual-candidate `<textarea>` with a self-contained CodeMirror 6 editor (`src/components/CandidateEditor.tsx`, `basicSetup` + `markdown()` + `indentWithTab`) on the `feature/v0.8-review-desk-workbench-foundation` branch. The candidate side now renders Markdown syntax highlighting, line numbers, Tab/Shift+Tab indentation, the active editor's theme/fontSize/tabSize/wrapLines, and spellcheck state. `useReviewDeskState` is unchanged so the existing candidate edit / tab / buffer safety guards all still bind to the same `value` + `onChange` contract; Compare / Apply remain explicit; the right-pane compare route, `DiffPane` / `FileCompareView` / `ChangeReviewView` shape, and `compareSource` / `compareTarget` slots are all untouched. Future v0.8 work (side-by-side workbench, live diff refresh, candidate source variants, source file selection, save policy) remains out of scope for this foundation slice.
- Latest v0.8 Review Desk compare/apply information design slice adds a preview meta row (Target / Buffer at compare / Candidate size / Compared at HH:MM:SS), an "Apply will mark unsaved" caption under the Apply button, and a stale preview banner with a Re-run Compare action on the `feature/v0.8-review-desk-compare-apply-clarity` branch. The candidate `CompareCase` now records `comparedAt: number`; `useReviewDeskState.runCandidateCompare` stamps it with `Date.now()`; and `ReviewSurface` derives a `CandidateStaleness` from `documentTabId` / `documentContents` / active-tab presence so Apply candidate is disabled while the preview is stale (no active tab / tab switched / buffer edited). The diff table is dimmed and `pointer-events: none` while the banner is shown. Compare / Apply remain explicit buttons, the existing candidate edit / tab / buffer safety guards still flow through `runCandidateCompare` / `setCandidateInputText` / `clearCandidate`, and the right-pane compare route, `DiffPane` / `FileCompareView` / `ChangeReviewView` shape, and `compareSource` / `compareTarget` slots are all untouched. Side-by-side workbench, live diff refresh, candidate source variants, source file selection, and save policy remain out of scope for this information design slice.
...
- The 2026-06-02 Review Desk discoverability and slash-keyboard polish pass adds a visible top-chrome `Review Desk` entry, a View menu `Review Desk` item, a `/` command for opening Review Desk, a `/` shortcut-list insertion command, and capture-phase Slash menu handling so `Enter` / `Tab` execute the selected command instead of inserting a newline.
...
- The next Review Desk UX direction is v0.8 work: an editable comparison workbench where the left side is the current buffer or a locked source file, the right side is a directly editable candidate, and any replace/save action first shows the target and change amount. Live diff refresh is desirable but not required for the first slice; Agent output should arrive as candidate text for review, while Agent Workbench remains a separate trust boundary and may later fit better as a detached window.
...
- Extended `src/types.ts` `CompareCase` `kind: "candidate"` with a new `comparedAt: number` field; `kind: "file"` and `kind: "changes"` branches are unchanged, so the right-pane compare route still consumes the same `kind: "candidate"` case unchanged.
...
3. For the next short automation window, route recurring work through post-v0.7 triage: one narrow quality check, reproduced post-release bug, documentation drift fix, or verified no-op per run. Move broader Review Desk UX work into the v0.8 lane.
