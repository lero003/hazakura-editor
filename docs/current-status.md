...
- Current release candidate is `v0.7.0` as a warning-expected DMG preview; the latest published public preview remains `v0.6.0` until the `v0.7.0` tag and GitHub Release are created.
- Current development lane is v0.7 Hazakura Review Desk MVP and release prep. The read-only Review Desk Readiness Gate is complete; use the remaining v0.7 window to close only tag-blocking Review Desk / Slash / Diff bugs, verify the MVP, and keep release notes honest rather than expanding the UX vision.
- Current automation emphasis is v0.7 release-prep hardening: prefer stale or failing quality gates, latest-HEAD built-app smoke gaps, reproduced MVP bugs, safety-boundary checks, and documentation drift before any feature expansion.
- The latest v0.7 release-prep rerun on 2026-06-02 passed `npm ci`, `npm run typecheck`, `cargo fmt --manifest-path src-tauri/Cargo.toml -- --check`, `cargo test --manifest-path src-tauri/Cargo.toml`, `npm run build:vite`, `npm run build`, `npm audit`, `cargo audit --file src-tauri/Cargo.lock` with temporary `CARGO_HOME`, and `git diff --check`. It could not complete release publication readiness in this managed environment because built-app launch through Launch Services still returned `kLSNoExecutableErr`, Computer Use access to `lab.hazakura.note` was denied, and `npm run build:dmg-preview` stopped at `hdiutil: create failed - 装置が構成されていません`.
...
3. For the next short automation window, route recurring work through the v0.7 release-prep hardening lane in `docs/development-automation.md`: one narrow quality gate, latest-HEAD built-app smoke gap, reproduced MVP bug, safety-boundary check, documentation drift fix, or verified no-op per run.
...
5. Keep v0.7 implementation limited to MVP release fixes. Move editable two-column Review Desk, candidate temporary-document policy, bounded Global Search, Command Palette, Settings consolidation, Frontmatter, KaTeX, and encoding conversion to v0.8+ unless the user explicitly reopens feature work before release.
...
8. Re-run latest-HEAD built-app smoke and warning-expected DMG creation in a host session where Launch Services, Computer Use, and `hdiutil` are available before creating the `v0.7.0` tag or GitHub Release.
