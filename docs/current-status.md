...
- Built local warning-expected DMG preview `hazakura-editor_0.6.0_aarch64-warning-expected.dmg`; SHA-256 is `1620ca7d134d787be63f3d6f2c8f4681cbeb9e2d8e9fed0b9109ab58eccdeb7e`.
...
- Post-push dependency audit on 2026-06-01 passed with `npm audit --json` reporting 0 vulnerabilities and `cargo audit --file src-tauri/Cargo.lock --json` reporting 0 vulnerabilities. `cargo audit` still reports informational warnings for transitive GTK3/unic/proc-macro-error/glib crates, and GitHub reported 1 moderate Dependabot alert after push, so the GitHub alert should be inspected before tagging.
...
- No v0.6 tag, GitHub Release publication, or remote asset verification has been performed yet. The GitHub repository has moved to `lero003/hazakura-editor`, and local `origin` points at `https://github.com/lero003/hazakura-editor.git`.
