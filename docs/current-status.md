...
5. The first recompute-trigger cleanup slice has landed; the second slice (Typewriter / IME stability, with a follow-up rAF race guard) has also landed; the third slice (visual-overlap fixture) has also landed; the fourth slice (task widget accessibility) has also landed. Continue from the `v0.14 Review Notes: 60 To 80 Point Ramp` section in `docs/l-mode-plan.md` with print / export boundary checks or a small performance baseline before deeper refactors.
