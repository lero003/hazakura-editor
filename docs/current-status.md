...
- Latest local App Store / TestFlight package candidate metadata
  ...
  `latest.json` instead. The prior `0.34.0` build `46` candidate is
  superseded for PDF output because TestFlight still showed macOS'
  print-unsupported alert. Raw App Store Connect upload, processing,
  TestFlight install / launch, and App Review logs are not tracked in
...
3. For the latest local App Store / TestFlight package candidate, see
   ...
   counter / pkg path / SHA-256 / generated time. The prior candidate is
   superseded for PDF output; it includes the v0.34 native PDF print
   attempt, v0.33 EPUB Export v1 Polish, the workspace marker and
   right-click slash-command source work, and the v1.0 RC pre-RC
   quality-slice planning pass. Upload, Apple processing, TestFlight
