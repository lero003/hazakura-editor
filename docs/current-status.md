...
- The current-file Outline pane lists ATX Markdown headings outside fenced code blocks, jumps the editor to the selected heading line, highlights the heading that contains the cursor line, and shows a cap note when a very large file has more headings than the outline displays.
