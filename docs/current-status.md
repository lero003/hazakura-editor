...
- Current package/app version: `0.36.0` across npm, Tauri, Cargo, and lockfile metadata.
...
- Latest local App Store / TestFlight package candidate metadata
  ...
  `50` with local package metadata and `smoke: skipped`. Raw App Store
  Connect upload, processing, TestFlight install / launch, and App
...
- Source-level `v0.36` e-book page-turn stabilization is implemented
  ...
  focused reader / chapter / EPUB tests and release preparation, and the
  current local App Store / TestFlight candidate metadata points at
  `0.36.0` build `50`; built-app long illustrated manuscript page-turn
  smoke, actual EPUB-reader page-break confirmation, TestFlight install /
  launch, and App Review remain outside this repository state until
  separately recorded.
...
3. For the latest local App Store / TestFlight package candidate, see
   ...
   records `0.36.0` build `50`; upload, Apple processing, TestFlight
   install / launch, and App Review handling remain explicit
   distribution-lane work outside this repository unless public-safe
