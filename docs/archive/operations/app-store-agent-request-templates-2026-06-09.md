...
対象範囲（優先順）:
1. 主要コントロールの到達性
- タブ・保存・設定・プレビュー・Export・Hazakura Local Assist 状態周りが
  キーボードで到達可能か
...
対象範囲:
1. 診断スコープ定義
- 収集対象: app version / build / macOS情報 / lane / Hazakura Local Assist可否 / sandbox再認可状態 / 主要エラー分類
...
- 主要画面（編集、Preview、Hazakura Local Assist、検索）の初期起動・操作時タイミング
