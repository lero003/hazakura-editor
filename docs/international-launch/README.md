...
これは設定変更の実施記録ではない。App Store Connectの認証済み画面、販売国、現行価格、契約、プライバシー回答は未確認。公開サイトの英語ページの稼働も確認できていない。取得できなかったページを「存在しない」「海外販売していない」とは判定していない。
...
メイン窓とLocal Assist窓を表示言語へ同期した。bundleの言語宣言とI-1は主要英語導線の監査と
上記判断が閉じた後に進める。英語コピーの存在だけで英語受け入れや海外販売の完了を主張しない。

## 1. 調査した現状

参照ソースは `main` の **`4d74b75a0ab2640471e6bc9977aec9785bcde148`**。以下はソースの確認であり、公開バイナリとの一致や今回の実機受入を意味しない。
...
同梱JSONは `publish_ready: false` の提案。`metadata` 以外の状態・条件・解説を掲載欄へ貼らない。`whats_new_version` は **3.0.3専用**で、別の提出版には流用せず変更履歴から書き直す。プロモーションテキストは170文字以下の任意素材で、Macの商品ページでの入力可否・表示を確認し、表示されなくても説明冒頭だけで価値が伝わる構成にしている。

このPRの検査対象は新規文書、JSONの構文・文字数・KeywordsのUTF-8バイト数・重複、文書内リンク、差分の空白と公開文の衛生。アプリのソース、依存、版数、署名設定は変更しない。npm/Rustビルド、署名、実機確認、ストアへの保存/送信、Web公開は実施していない。ローカルGit取得は環境の名前解決で失敗したため、ソース参照とPR作成はGitHub接続を使用し、ローカル差分検査は新規ファイルの作業用Gitディレクトリで行う。
