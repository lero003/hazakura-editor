...
export type PreferencesCopy = {
  ...
  /** 何を送らない・何を勝手にしないかの境界（モック17）。 */
  assistBoundaries: { title: string; body: string }[];
  assistCapabilityDevice: string;
  assistNotice: string;
  // オンデバイスモデルの独立ページ。見出し、Settings 本文からの入口、
  // 保存先の説明（Apple-hosted 配布で保存先は選べない）に使う。
...
export function getPreferencesCopy(lang: MenuLanguage): PreferencesCopy {
  if (isKanaStyle(lang)) {
    return {
      ...
      assistBoundaries: [
        { title: "そとの AI に おくりません", body: "くらうどの もでるへ じどうで きりかへません。" },
        { title: "はんえい するのは、じぶん", body: "ていあんを みてから、ふみに はんえいします。" },
        { title: "かってに ほぞん しません", body: "はんえいと ほぞんは、べつの そうさです。" },
        { title: "たいしょうは、えらんだ ぶんしょう", body: "ふぉるだ ぜんたいを じどうで よみません。" },
      ],
      assistCapabilityDevice: "この Mac の もでるで つくります。",
  ...
  return isJapaneseMenuLanguage(lang)
    ? {
        ...
        assistBoundaries: [
          { title: "外部AIへ送信しない", body: "クラウドモデルへの自動切り替えはありません。" },
          { title: "反映するのは、自分で", body: "提案を確認してから、文書に反映します。" },
          { title: "勝手に保存しない", body: "反映と保存は、別々の操作です。" },
          { title: "対象は、選んだ文章", body: "ワークスペース全体を自動解析しません。" },
        ],
        assistCapabilityDevice: "このMacのオンデバイスモデルで生成します。生成した提案は、本体の差分で確認します。",
    ...
    : {
        ...
        assistBoundaries: [
          { title: "Never sent to an external AI", body: "There is no automatic switch to a cloud model." },
          { title: "You apply it yourself", body: "The proposal is reviewed before it reaches the document." },
          { title: "No silent saves", body: "Applying and saving are separate actions." },
          { title: "Only the text you select", body: "The whole workspace is never scanned automatically." },
        ],
        assistCapabilityDevice: "Generated by this Mac's on-device model. Proposals are reviewed as a diff in the main window.",
