...
The App Store submission build may bundle Hazakura Local Assist as an on-device writing companion. It can use Apple Intelligence and, when a separately published model is available, an allowlisted Core AI model stored on this Mac. The current build contains the Core AI adapter and model-management screen but no published Core AI model pack. It reports that state and keeps Apple Intelligence as the only selectable model. The app does not send your document or Hazakura Local Assist requests to external AI services or external APIs, and there is no network inference fallback. Conversation and proposals do not edit the document automatically: you review the proposal and explicitly apply it in the main window. Applied text remains unsaved and can be undone. Opening Local Assist settings may check availability without generating text.

If a Core AI model is published in a later candidate, downloading or removing it will require an explicit action in settings and will use an allowlisted Apple-hosted asset pack. Model acquisition is separate from inference: after installation, generation stays on-device. The app does not accept an arbitrary model URL, filesystem path, or user-supplied model package.
