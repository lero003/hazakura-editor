...
function managerCopy(language: MenuLanguage) {
  if (language === "en") return {
    ...
    notPublished: "No Core AI model has been published for download yet. This build includes the adapter and management controls, but continues to use Apple Intelligence.",
  ...
  if (language === "kana") return {
    ...
    notPublished: "Core AI の もでるは まだ くばってゐません。この あぷりには うけいれと かんりの しくみだけが あり、いまは Apple Intelligence を つかひます。",
  ...
  return {
    ...
    notPublished: "Core AI モデルはまだ配布されていません。このビルドには実行アダプタと管理画面だけが入り、現在は Apple Intelligence を使います。",
