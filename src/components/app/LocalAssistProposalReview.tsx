...
function getProposalReviewCopy(lang: MenuLanguage) {
  if (isKanaStyle(lang)) return {
    blocked: "せいせい・ていしの しょりが おわるまで、はんえい・はきは できません。",
    model: "つくった モデル", unknownModel: "ふめい", fixtureModel: "どうさ かくにんよう",
    coreAITestModel: "Core AI（Qwen3 0.6B どうさ かくにんよう）",
  ...
  if (isJapaneseMenuLanguage(lang)) return {
    blocked: "生成・停止処理が終わるまで、反映・破棄はできません。",
    model: "生成元", unknownModel: "不明", fixtureModel: "動作確認用",
    coreAITestModel: "Core AI（Qwen3 0.6B 動作確認用）",
  ...
  return {
    blocked: "Wait for generation or cancellation to finish before applying or discarding.",
    model: "Generated by", unknownModel: "Unknown", fixtureModel: "Test fixture",
    coreAITestModel: "Core AI (Qwen3 0.6B test fixture)",
...
export function LocalAssistProposalReview({ activeTab, menuLanguage, fontSize, blocked = false, onApply, onDiscard, onReturnToEditing }: LocalAssistProposalReviewProps) {
  ...
  const generatedBy = proposal.generation?.modelId === "apple:foundation-models:system-default"
    ? "Apple Intelligence"
    : proposal.generation?.modelId === "apple:core-ai:qwen3-0.6b-test"
      ? copy.coreAITestModel
      : proposal.generation?.modelId === "fixture:helper-v0.12"
        ? copy.fixtureModel
        : copy.unknownModel;
  ...
  return (
    <div data-review-request-id={proposal.requestId} aria-label={copy.regionLabel} aria-busy={applying || !!proposal.streaming}
      ...
      <div className="local-assist-proposal-review-header">
        ...
        <h2 className="local-assist-proposal-review-title">{copy.regionLabel}</h2>
      ...
      {proposal.streaming ? <p role="status">{copy.busy}</p> : <>
        <p className="local-assist-proposal-review-summary">{copy.model}: {generatedBy}</p>
